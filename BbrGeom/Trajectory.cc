...
// 	$Id: Trajectory.cc 644 2010-06-01 20:33:21Z brownd $
// 
// Description:
...
{
  if (hilim < lowlim) {
    ErrMsg(error) << "Invalid Trajectory range requested in new trajectory." << endmsg;
    flightrange[0] = hilim;
...
Trajectory::setFlightRange(double newrange[2])
{
  if (newrange[1] >= newrange[0]) {
    ...
    ErrMsg(error) << "Invalid Trajectory range requested in setFlightRange." << endmsg;
