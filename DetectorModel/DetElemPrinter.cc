...
DetElemPrinter::print(ostream& o) const
{
  ...
  for (DetElemList::const_iterator theElemIterator = theElemList.begin(); theElemIterator
      != theElemList.end(); ++theElemIterator) {
  ...
  for (std::map<const DetType*, DetElemList*>::const_iterator theHashIter =
      theHash.begin(); theHashIter != theHash.end(); ++theHashIter) {
    ...
    if (type != 0) {
      for (DetElemList::const_iterator elemIter = elemList->begin(); elemIter
          != elemList->end(); ++elemIter) {
...
//        Action firstPoint = iter.next(aVertex);
        ...
        if (whatNext == Continue) {
          while (1) {
            if (whatNext == CloseLine || whatNext == CloseShape) move = true;
