...
double DetMaterial::_dgev = 0.000153536;
double DetMaterial::_minkappa(1.0e-3);
double DetMaterial::_scatterfrac(0.999); // integrate 99.9% percent of the tail by default, this should be larger
...
// compute cached values; these are used in detailed scattering models
  ...
  _chic2 = 1.57e-7*_zeff*(_zeff+1)/_aeff;  
  _chia2_1 = 2.007e-11*pow(_zeff,2.0/3.0);
...
DetMaterial::highlandSigma(double mom,double pathlen, double mass) const {
  if(mom>0.0){
    ...
    return 0.015*sqrt(radfrac)/(mom*particleBeta(mom,mass));
  } else
    return 1.0;
}
