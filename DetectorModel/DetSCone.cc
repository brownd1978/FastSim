...
sconeMax(double x, double y)
{
  return x > y ? x : y;
}

static inline double
sconeMin(double x, double y)
{
  return x < y ? x : y;
}
...
public:
  DetSConeEquation(const DetSCone* cone, const Hep3Vector& r,
      const Hep3Vector& vu);
  int
  findRoots(double x[2]) const;
  int
  findExtrema(double x[2], double dist[2]) const;
  double
  a() const
  {
    return _a;
...
//
inline
DetSConeEquation::DetSConeEquation(const DetSCone* cone, const Hep3Vector& r,
    const Hep3Vector& vu) :
...
DetSCone::updateCache()
{
  ...
  _cosAlpha = (_tanAlpha < 0.0) ? -1 / sqrt(1 + _tanAlpha * _tanAlpha) : 1
      / sqrt(1 + _tanAlpha * _tanAlpha);
  _sinAlpha = _tanAlpha * _cosAlpha; // positive
}

DetSCone::DetSCone(const HepTransformation& t, double tanAlpha, double vertex,
    double scale) :
...
DetSCone::DetSCone(const HepTransformation& t, double z1, double z2, double r1,
    double r2, double scale) :
...
DetSCone::normTo(const HepPoint& x, Hep3Vector& n) const
{
  ...
  case outsideCone:
    ...
    phi = r.phi();
    n
        = gotoGlobal(Hep3Vector(uCosA() * cos(phi), uCosA() * sin(phi),
            -sSinA()));
    break;
  case onAxis:
...
DetSCone::firstDeriv(const HepPoint& x, const Hep3Vector& v) const
{
  ...
  case outsideCone:
    ...
    return -vu.z() * sSinA() + (vu.x() * r.x() + vu.y() * r.y()) * uCosA()
        / r.perp();
  case onAxis:
...
chooseRoot(const int roots, double x[2], const DetSurface::intermode mode,
    const DetSurface::intertype goodOne, double& dist)
{
  switch (roots) {
...
DetSCone::distTo(const HepPoint& p, const Hep3Vector& v, double& dist,
...
{
  os << " S(ingle)Cone Surface with" << " tan=" << _tanAlpha << " z0=" << _z0
      << " scale=" << _scale << endl;
} // end print
...
DetSCone::printAll(ostream& os) const
{
  ...
  os << "Origin = " << origin.x() << " " << origin.y() << " " << origin.z()
      << endl << "Axis direction = " << axis.x() << " " << axis.y() << " "
      << axis.z() << endl << "Local X direction = " << xaxis.x() << " "
      << xaxis.y() << " " << xaxis.z() << endl;
} // end printAll
