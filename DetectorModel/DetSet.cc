...
//
void
dsElemPrint(const DetElem* elem, void* os)
{
...
void
dsSetPrintAll(const DetSet* set, void* os)
{
  ostream* stream = (ostream*) os;
  set->printAll(*stream);
}
...
bool
dsSelIDFun(const DetElem* elem, void* data)
{
...
{
  os << "Detector Set " << _dsname << " " << _dsnum << " has " << _dlist.size()
      << " direct elements and " << _slist.size() << " direct subsets";
  if (isReady()) os << " and is Ready for intersections" << endl;
  else os << " and is NOTReady for intersections" << endl;
}
//
//  Print; this uses two list functions to access the elements.
//
...
DetSet::printAll(ostream& os) const
{
  ...
  } else os << "Detector Set " << _dsname << " " << _dsnum
  ...
  } else os << "and contains no subsets " << endl;
}
//
...
void
DetSet::select(DetElemList& theList, elemSelFun efun, void* elemdata,
    setSelFun sfun, void* setdata, bool clear) const
{
  //
  //  clear the list at the top level call
  //
  if (clear) theList.clear();
  //
  ...
  if (!sfun || (*sfun)(this, setdata)) {
    for (DetElemList::const_iterator diter = _dlist.begin(); diter
        != _dlist.end(); ++diter) {
      DetElem* match = *diter;
      if (efun(match, elemdata)) theList.push_back(match);
    }
  }
  //
  //  Recursively select on the contained sets, WITHOUT clearing the list!
  //
  for (DetSetList::const_iterator siter = _slist.begin(); siter != _slist.end(); ++siter) {
...
//
void
DetSet::setSelect(DetSetList& theList, setSelFun sfun, void* setdata,
    bool clear) const
{
  //
  //  clear the list at the top level call
  ...
  //  Check this set
  //
  if ((*sfun)(this, setdata)) theList.push_back((DetSet*) this);
  //
  //  Recursively select on the contained sets, WITHOUT clearing the list!
  //
  for (DetSetList::const_iterator siter = _slist.begin(); siter != _slist.end(); ++siter) {
...
//
bool
DetSet::nextIntersection(const Trajectory* traj, DetIntersection& nextinter,
    DetIntersection* lastinter) const
{
  ...
  } else return firstIntersection(traj, nextinter);
}

bool
DetSet::firstIntersection(const Trajectory* traj, DetIntersection& firstinter,
    double* myrange) const
{
  ...
  if (_dlist.size() > 0) {
    ...
    // reset the range
    if (intersected) trajrange[1] = current.pathrange[1];
  }
  //
...
DetSet::intersection(std::vector<DetIntersection>& divec,
    const Trajectory* traj, double* myrange, bool clear) const
{
  //
  //  Make sure the set is ready
  //
    ...
    while (diter != dlist().end()) {
      ...
      while (elem->intersect(traj, dinter) && ninter < _MAXINTER) {
        ...
        if (dinter.pathrange[0] < dinter.pathrange[1]) {
          ...
          if (dinter.pathrange[1] >= range[1]) break;
          // setup for the next intersectoin
          dinter.pathrange[0] = dinter.pathrange[1] + _epsilon;
          dinter.pathrange[1] = std::max(dinter.pathrange[0] + _epsilon,
              range[1]);
        } else {
          ErrMsg(error)
              << "DetSet: exit point comes before entrance in element interection."
              << endmsg;
          break;
        }
      }
...
{
  ErrMsg(warning) << "This is an obsolete function: please convert to call "
      << endl << "DetSet::applyLocal(std::vector<DetAlignElem>) instead"
      << endmsg;
...
{
  ErrMsg(warning) << "This is an obsolete function: please convert to call "
      << endl << "DetSet::removeLocal(std::vector<DetAlignElem>) instead"
      << endmsg;
  std::vector<DetAlignElem> elems;
  for (unsigned ielem = 0; ielem < nelem; ielem++)
    elems.push_back(alist[ielem]);
...
//
void
dsGnuplotElement(const DetElem* elem, void* plot)
{
