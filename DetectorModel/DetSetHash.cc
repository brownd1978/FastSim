...
DetSetHash::DetSetHash(const DetSet& theSet, Ehash elemHash, elemSelFun esel,
    void* eseldat) :
  _hashtable(0), _nindex(0), _maxindex(-_MAXINDEX), _minindex(_MAXINDEX),
      _ehash(elemHash)
{
  //
...
DetSetHash::findElement(int elemidnum)
{
  ...
  if (index >= _minindex && index <= _maxindex && _hashtable[index] != 0
      && _hashtable[index]->elementNumber() == elemidnum) return _hashtable[index];
  else {
    ErrMsg(error) << "DetSetHash::findElement invalid element ID number"
        << endmsg;
    return 0;
  }
}
