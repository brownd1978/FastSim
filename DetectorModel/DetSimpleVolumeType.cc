...
DetSimpleVolumeType::DetSimpleVolumeType(const char* typeName, int typeId,
    const DetMaterial* theMaterial, std::vector<HepPoint>& theCorners) :
  DetVolumeType(typeName, typeId), _material(theMaterial)
{
  ...
  for (i = 0; i < nCorners; i++) {
    ...
    thePointStore->push_back(new ThreeDCoord(thePoint->x(), thePoint->y(),
        thePoint->z()));
  }
  ...
  for (side = 0; side < nSides; side++) {
    ...
    Hep3Vector sideOne = *sideCorners[0] - *sideCorners[1], sideTwo =
        *sideCorners[2] - *sideCorners[1], normal = sideOne.cross(sideTwo);

    Hep3Vector centre;

    for (i = 0; i < nEdgesThisSide; i++)
      centre += Hep3Vector(sideCorners[i]->x(), sideCorners[i]->y(),
          sideCorners[i]->z());
    ...
    for (i = 0; i < nEdgesThisSide; i++) {
      ...
      Hep3Vector dummy;
      (*mySides())[side]->normalTo(*sideCorners[i], dummy, *aPoint);
