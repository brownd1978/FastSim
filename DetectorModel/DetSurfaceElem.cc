...
DetSurfaceElem::DetSurfaceElem(DetSurfaceType* type, const char* oname, int id,
  ...
  DetElem(type, oname, id), _osurf(surf.copyOf())
{
...
DetSurfaceElem::intersect(const Trajectory* trktraj, DetIntersection& dinter) const
{
  ...
  while (iflag.success()) {
    ...
    if (jflag) {
      ...
      dinter = DetIntersection((DetElem*) this, trktraj, flightdist, flightdist
          - thickness / 2, flightdist + thickness / 2);
      dinter.flag[0] = -2;
      dinter.flag[1] = -2;
      //
      break;
      ...
      iflag
          = intersection.intersect(flightdist, surfcoord, trkOut, flightrange);
    }
  }
  return jflag;
}
