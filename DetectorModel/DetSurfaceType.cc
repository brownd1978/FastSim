...
DetSurfaceType::DetSurfaceType(const char* name, int ityp, std::vector<
    TwoDCoord>& theCorners) :
  DetType(name, ityp)
{
...
//
double
DetSurfaceType::effectiveThickness(const TwoDCoord* point, double dircos1,
    double dircos2) const
{
  return thickness(point) / sqrt(1.0 - (pow(dircos1, 2) + pow(dircos2, 2)));
}
...
DetSurfaceType::insideLimitsOf(const TwoDCoord& thisPoint) const
{
  ...
  for (i = 0; i < (numberOfCorners - 1); i++) {
    ...
    if (!insideLine(thisPoint, *point1, *point2)) return false;
  }
...
bool
DetSurfaceType::insideLine(const TwoDCoord& toTest, const TwoDCoord& p1,
    const TwoDCoord& p2) const
{
  ...
  if (p2[0] == p1[0]) {
    ...
    // as inside the line.)
    if ((xpt >= 0 && toTest[0] >= 0 && xpt >= toTest[0]) || (xpt < 0
        && toTest[0] < 0 && xpt < toTest[0]) || (xpt < 0 && toTest[0] >= 0)
