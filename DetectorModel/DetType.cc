...
//----------------
DetType::DetType() :
  _itype(-1), _tname("Unknown"), _outline(new std::vector<TypeCoord*>),
      _pointStore(new std::vector<TypeCoord*>)
{
}

DetType::DetType(const char* name, int ityp) :
  _itype(ityp), _tname(name), _outline(new std::vector<TypeCoord*>),
      _pointStore(new std::vector<TypeCoord*>)
{
}
...
DetType::operator==(const DetType& otherType) const
{
  ...
  if (_itype == otherType._itype && _tname == otherType._tname) answer = true;
