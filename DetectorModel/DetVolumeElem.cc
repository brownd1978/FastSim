...
DetVolumeElem::DetVolumeElem(DetVolumeType* itsType, const char* name, int id,
  ...
  DetElem(itsType, name, id), _sides(new std::vector<DetSurface*>)
{
...
//-------------
int
DetVolumeElem::intersect(const Trajectory* traj, DetIntersection& dinter) const
{
  ...
  if (ilist.size() == 2) {
    ...
    if (entrance < dinter.pathrange[0] && exit > dinter.pathrange[1]) {
      entrance = dinter.pathrange[0];
      exit = dinter.pathrange[1];
      double path = 0.5 * (entrance + exit);
      dinter = DetIntersection((DetElem*) this, traj, path, entrance, exit);
...
void
DetVolumeElem::sideIntersect(const Trajectory* traj, std::vector<
...
DetVolumeElem::physicalOutline(std::vector<HepPoint>& pvec) const
{
  ...
  const std::vector<TypeCoord*>* theOutline = detectorType()->pointStore();
  //
  ...
  for (int icorn = 0; icorn < npoints; icorn++) {
    ...
    HepPoint typePoint((*typeCoord)[0], (*typeCoord)[1], (*typeCoord)[2]);
