...
//------------------------------------------------------------------------
  ...
  const double denom2 = pow(pYx,2) + pow(pXy,2);
  ...
  derivs[TrkExchangePar::ex_z0][pX] = 
    mom.z()*(pow(mom.x(),2)*pos.x() + 2*mom.x()*mom.y()*pos.y() -
	     pow(mom.y(),2)*pos.x() + A*mom.y()*r2) / (pT2 * denom2);
  // z0 wrt pY
  derivs[TrkExchangePar::ex_z0][pY] = 
    -mom.z()*(pow(mom.x(),2)*pos.y() - 2*mom.x()*mom.y()*pos.x() -
	      pow(mom.y(),2)*pos.y() + A*mom.x()*r2) / (pT2 * denom2);
  // z0 wrt pZ
...
//------------------------------------------------------------------------
  ...
  const double phi0 = params.phi0();
  const double tanDip = params.tanDip();
  const double cosDip = 1./sqrt(1. + pow(tanDip,2));
