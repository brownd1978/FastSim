...
KalRep::KalRep(TrkSimpTraj* seed, TrkHotList* hotl,
...
KalRep::KalRep(TrkSimpTraj* seed, TrkDifPieceTraj* rtraj, TrkHotList* hotl,
	const std::vector<DetIntersection>& dinters,
	TrkRecoTrk* trk, const KalContext& context,
	PdtPid::PidType hypo) : TrkRep(hotl,trk,hypo,true), _maxdist(0),_maxfltdif(0),
	_niter(0),_ninter(0),_ptraj(0),_reftraj(rtraj),_integrator(0),
	_kalcon(&context),
	_seedtraj(seed),
	_stopsite(0)
{
...
// convert the intersections to use the new reftraj;
		std::vector<DetIntersection> tlist(dinters);
		for(std::vector<DetIntersection>::iterator iinter = tlist.begin();iinter!= tlist.end();iinter++)
		  if(iinter->trajet != _reftraj)
      ErrMsg(error) << "incorrect trajectory reference in det intersection " << endmsg;
		buildMaterialSites(_fitrange,tlist);
  }
// build the bend sites
  if(_kalcon->bendSites()){
...
// trajectory convergence  
    if(converged) {
      converged = _maxdist < _kalcon->distanceTolerance();

// momentum convergence
      if(converged) {
        converged = fabs(estimatedMomDiff()) < _kalcon->maxMomDiff();

//  Parameter convergence; if the parameter differences tolerance is set < 0.0  skip the test
        if(converged) {
          converged = _kalcon->maxParamDiff(trkOut) < 0.0 ||
...
// estimated change in momentum scales as mass^2/momentum^3
  ...
  double ddmom = dedxfactor*dmom*energy*pow(Pdt::mass(particleType()),2)/
    pow(avgmom,4);
  return ddmom;
}
...
// only cleanup if necessary
  if(hotList()->hitCapable()){
    std::unique_ptr<TrkHotList> old( _hotList.release() );
