...
// 	$Id: MsgLoggerImpl.cc 518 2010-01-15 08:25:10Z stroili $
//
// Description:
//	Class MsgLoggerImpl
//
...
namespace {
  ...
  // loggers
  typedef std::map< std::string, boost::shared_ptr<MsgLoggerImpl> > Name2ImplMap ;
  struct Name2Impl {
  ...
  void initRoot ( MsgLoggerImpl& root ) {
    root.addHandler ( new MsgHandlerStdStreams ) ;
  }
...
namespace {
  ...
  void makeLogger ( const std::string& name, const std::string level, bool propagate ) 
  {
    ...
    if ( it != implementations.map.end() ) {
      if ( name.empty() ) {
        ErrMsg(error) << "MsgLogger::makeLogger - root logger already exists." << endmsg ;
      } else {
        ErrMsg(error) << "MsgLogger::makeLogger - logger '" << name << "' already exists." << endmsg ;
      }
      return ;
    }

    MsgLogLevel s ( MsgLogLevel::debug ) ;
    if ( level == "info" ) {
      s = MsgLogLevel::info ;
    } else if ( level == "trace" ) {
      s = MsgLogLevel::trace ;
      ...
      ErrMsg(error) << "MsgLogger::makeLogger - unknown level name: " << level << ", fallback to debug." << endmsg ;
    }

    boost::shared_ptr<MsgLoggerImpl> logger ( new MsgLoggerImpl( name ) ) ;
    logger->addHandler ( new MsgHandlerStdStreams ) ;
    logger->setLevel ( s ) ;
