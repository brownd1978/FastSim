...
//      $Id: Pdt.cc 831 2010-11-08 14:15:03Z stroili $
//
// Description:
...
PdtLund::LundType
Pdt::antiCode(PdtLund::LundType kf_id)
{
...
void
Pdt::addParticle(const char *pname, PdtLund::LundType id, float spin,
    float charge, float mass, float width, float cut)
{
...
void
Pdt::addParticle(const char *pname, PdtGeant::GeantType id, float spin,
    float charge, float mass, float width, float cut)
{
...
void
Pdt::addDecay(const char* pname, float bf, const char* child1,
    const char* child2, const char* child3, const char* child4,
    const char* child5)
{
...
void
Pdt::addDecay(PdtLund::LundType id, float bf, PdtLund::LundType child1,
...
PdtEntry*
Pdt::lookup(const std::string& name)
{
  return (*_entriesn)[name];
}

PdtEntry*
Pdt::lookup(PdtLund::LundType id)
{
...
PdtEntry*
Pdt::lookup(PdtGeant::GeantType id)
{
...
PdtEntry*
Pdt::lookup(PdtPid::PidType id, int charge)
{
...
PdtEntry*
Pdt::lookup(PdtPid::PidNeutralType id, int charge)
{
...
void
Pdt::printOn(ostream& cout)
{

  //lange move printouts here
  cout << "Number of particles in tables: " << endl << "By pdt number   :"
      << _entries->size() << endl << "By name         :" << _entriesn->size()
      << endl << "By geant number :" << _entriesg->size() << endl
...
void
Pdt::readMCppTable(const char *filenm)
{
...
PdtLund::LundType
Pdt::lundId(const PdtGeant::GeantType gId)
{

  if (gId == PdtGeant::deuteron || gId == PdtGeant::tritium || gId
      == PdtGeant::alpha || gId == PdtGeant::geantino || gId == PdtGeant::He3
...
PdtGeant::GeantType
Pdt::geantId(const PdtLund::LundType lId)
{
  ...
  //
  if (lId == PdtLund::nu_e || lId == PdtLund::anti_nu_e || lId
      == PdtLund::nu_mu || lId == PdtLund::anti_nu_mu || lId == PdtLund::nu_tau
...
PdtPid::PidType
Pdt::pidId(const PdtLund::LundType lId)
{
...
Pdt::pidNeutId(const PdtLund::LundType lId)
{
...
PdtPid::PidType
Pdt::pidId(const PdtGeant::GeantType gId)
{
  return pidId(lundId(gId));
}

PdtPid::PidNeutralType
Pdt::pidNeutId(const PdtGeant::GeantType gId)
{
  return pidNeutId(lundId(gId));
}

PdtGeant::GeantType
Pdt::geantId(const PdtPid::PidType pId, int charge)
{
  return geantId(lundId(pId, charge));
}

PdtGeant::GeantType
Pdt::geantId(const PdtPid::PidNeutralType pId, int charge)
{
  return geantId(lundId(pId, charge));
}

PdtLund::LundType
Pdt::lundId(const PdtPid::PidType pId, int charge)
{
...
PdtLund::LundType
Pdt::lundId(const PdtPid::PidNeutralType pId, int charge)
{
...
PdtLund::LundType
Pdt::lundId(const PdtPdg::PdgType pdg)
{
...
PdtPdg::PdgType
Pdt::pdgId(const PdtLund::LundType lund)
{
...
unsigned
Pdt::PdtNameHash(const string& c)
{
...
bool
Pdt::sameOrConj(PdtLund::LundType id1, PdtLund::LundType id2)
{
  return Pdt::sameOrConj(Pdt::lookup(id1), Pdt::lookup(id2));
}

bool
Pdt::sameOrConj(PdtPdg::PdgType id1, PdtPdg::PdgType id2)
{
  return Pdt::sameOrConj(Pdt::lookup(id1), Pdt::lookup(id2));
}

bool
Pdt::sameOrConj(PdtEntry* pdt1, PdtEntry* pdt2)
{
