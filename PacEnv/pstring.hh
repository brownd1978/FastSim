 ...
 * The interface is modeled off Python's builtin string type.
 */
...
public:
    ...
    pstring head(size_type n = 1) const;
    pstring tail(size_type n = 1) const;
    ...
    pstring firstline(size_type m = 0) const;
...
protected:
//    friend void clamp(int& i, size_type min, size_type max);
};

/* Conversion functions for creating strings from other data types */
pstring str(bool b);
pstring str(int i);
pstring str(long l);
pstring str(float f);
