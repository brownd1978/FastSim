...
std::vector<EvtParticleDecayList> PacDecayTable::_decaytable;

int PacDecayTable::getNMode(int ipar){
   return _decaytable[ipar].getNMode();
} 

EvtDecayBase* PacDecayTable::getDecay(int ipar, int imode){
  return _decaytable[ipar].getDecayModel(imode);
}

void PacDecayTable::printSummary(){
...
EvtDecayBase* PacDecayTable::getDecayFunc(EvtParticle *p){
  int partnum;
...
void PacDecayTable::readDecayFile(const std::string dec_name, bool verbose){

  if ( _decaytable.size() < EvtPDL::entries() ) _decaytable.resize(EvtPDL::entries());
  EvtModel &modelist=EvtModel::instance();
  int i;

  report(INFO,"PacDecayTable") << "In readDecayFile, reading:"<<dec_name.c_str()<<endl;
  
  ifstream fin;
  
  fin.open(dec_name.c_str());
  if (!fin) {
    report(ERROR,"PacDecayTable") << "Could not open "<<dec_name.c_str()<<endl;
  }
  fin.close();
  ...
  if (!hasend){
    report(ERROR,"PacDecayTable") << "Could not find an 'End' in "<<dec_name.c_str()<<endl;
    report(ERROR,"PacDecayTable") << "Will terminate execution."<<endl;
    ::abort();
  }
  ...
  do{
	...
	report(INFO,"PacDecayTable") 
      ...
      if (id==EvtId(-1,-1)) {
	report(ERROR,"PacDecayTable") <<"Unknown particle name:"<<oldname.c_str()
	...
	if (nameTemp!=";") {
      ...
      if (a==EvtId(-1,-1)) {
	report(ERROR,"PacDecayTable") <<"Unknown particle name:"<<aname.c_str()
			       <<" on line "<<parser.getLineofToken(itoken)<<endl;
	report(ERROR,"PacDecayTable") <<"Will terminate execution!"<<endl;
	::abort();
      }
      ...
      if (abar==EvtId(-1,-1)) {
	report(ERROR,"PacDecayTable") <<"Unknown particle name:"<<abarname.c_str()
			       <<" on line "<<parser.getLineofToken(itoken)<<endl;
	report(ERROR,"PacDecayTable") <<"Will terminate execution!"<<endl;
	::abort();
      }
      ...
      if (ipar==EvtId(-1,-1)) {
	report(ERROR,"PacDecayTable") <<"Unknown particle name:"<<name.c_str()
			       <<" on line "
	...
	_decaytable[ipar.getAlias()].removeDecay();
      ...
      if ( verbose )
	report(INFO,"PacDecayTable") << pname.c_str() << endl;
      ...
      if (verbose )
	report(INFO,"PacDecayTable") << "Changing particle properties of " <<
	...
	report(DEBUG,"EvtGen") <<"Refined maximum mass for " << EvtPDL::name(thisPart).c_str() << " to be " << tmass << endl;
      ...
      pname=parser.getToken(itoken++);
      bool yesno=false;
      if ( parser.getToken(itoken++).compare("yes") == 0 ) yesno=true;
      EvtId thisPart = EvtPDL::getId(pname);
	...
	report(DEBUG,"EvtGen") <<"Redefined Partial wave for " << pname.c_str() << " to " << pnameD1.c_str() << " " << pnameD2.c_str() << " ("<<pw<<")"<<endl;
      ...
      if (ipar==EvtId(-1,-1)) {
	report(ERROR,"PacDecayTable") <<"Unknown particle name:"<<parent.c_str()
			       <<" on line "
			       <<parser.getLineofToken(itoken-1)<<endl;
	report(ERROR,"PacDecayTable") <<"Will terminate execution!"<<endl;
	::abort();
	...
	_decaytable[ipar.getAlias()].removeDecay();
	  ...
	  while (token.c_str()[i++]!=0){
	    if (isalpha(token.c_str()[i])){
	      report(ERROR,"PacDecayTable") << 
		"Expected to find a branching fraction or Enddecay "<<
		"but found:"<<token.c_str()<<" on line "<<
		parser.getLineofToken(itoken-1)<<endl;
	      report(ERROR,"PacDecayTable") << "Possibly to few arguments to model "<<
	  ...
	  int isname=EvtPDL::getId(parser.getToken(itoken)).getId()>=0;
	    ...
	    report(INFO,"PacDecayTable") << parser.getToken(itoken).c_str()
	     << " is neither a particle name nor "
	     << "the name of a model. "<<endl;
	    report(INFO,"PacDecayTable") << "It was encountered on line "<<
	      parser.getLineofToken(itoken)<<" of the decay file."<<endl;
	    report(INFO,"PacDecayTable") << "Please fix it. Thank you."<<endl;
	    report(INFO,"PacDecayTable") << "Be sure to check that the "
	     << "correct case has been used. \n";
	    report(INFO,"PacDecayTable") << "Terminating execution. \n";
	    ::abort();
	  ...
	  while(EvtPDL::getId(parser.getToken(itoken)).getId()>=0){
	    ...
	    if (daught[n_daugh-1]==EvtId(-1,-1)) {
	      report(ERROR,"PacDecayTable") <<"Unknown particle name:"<<sdaug.c_str()
				     <<" on line "<<parser.getLineofToken(itoken)<<endl;
	      report(ERROR,"PacDecayTable") <<"Will terminate execution!"<<endl;
	      ::abort();
	    }
      ...
      report(ERROR,"PacDecayTable") << "Found unknown command:'"<<token.c_str()<<"' on line "
			     <<parser.getLineofToken(itoken)<<endl;
      report(ERROR,"PacDecayTable") << "Will terminate execution!"<<endl;
      ::abort();
  ...
  for ( ii=0; ii<EvtPDL::entries(); ii++){
    ...
    if ( minMass > EvtPDL::getMinMass(temp) ) {
      if ( verbose )
	report(INFO,"PacDecayTable") << "Given allowed decays, resetting minMass " << EvtPDL::name(temp).c_str() << " " 
...
int  PacDecayTable::findChannel(EvtId parent, std::string model, 
...
int  PacDecayTable::inChannelList(EvtId parent, int ndaug, EvtId *daugs){
