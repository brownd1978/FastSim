...
const double TrkGammaVertex::_awconv(1.0/pow(0.0062,2));
const double TrkGammaVertex::_awdalitz(1.0/pow(0.057,2));
...
// compute TRANSVERSE flightlength and space flightlength
      ...
      flt[itraj] = tflt[itraj]*sqrt(1.0+pow(htraj->tanDip(),2));
...
// pre-compute some common values
  ...
  double invcosdip2 = pow(invcosdip,2);
  ...
  */
  ...
  for (int iind=0; iind<ngind; ++iind) {
    for (int j=0; j<nSample; ++j) {
      ...
      chisq += pow(dphi*cos(gpar[dip]),2)*aweight;
      chisq += pow(gpar[ddip],2)*aweight;
...
// right-wrap parameters
  ...
  if(fabs(phi1 - trkpars[0][HelixTraj::phi0Index]) < Constants::pi)
  ...
  if(fabs(phi2- trkpars[1][HelixTraj::phi0Index]) < Constants::pi)
...
// now momentum.
  ...
  double emass2 = pow(getElectronPdt()->mass(),2);
  HepLorentzVector g4;
...
// pt is given just by the curvature
    ...
    double invcosdip2 = 1.0+pow(tandip,2);
