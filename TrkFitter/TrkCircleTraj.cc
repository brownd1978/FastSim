...
//      $Id: TrkCircleTraj.cc 104 2010-01-15 12:13:14Z stroili $
//
// Description:
//
// Environment:
...
{ 
  ...
  if( pvec.num_row() != nCirPrm() ||
    ...
    ErrMsg(fatal) << 
      "CircleTraj: incorrect constructor vector/matrix dimension" << endmsg;
  }
...
TrkCircleTraj::TrkCircleTraj(const TrkExchangePar& inpar,
                             double lowlim, double hilim, const HepPoint& refpoint) :
        TrkSimpTraj(inpar.params().sub(1,3), 
                    inpar.covariance().sub(1,3), lowlim,hilim,refpoint)
{
  if (omega() == 0) parameters()->parameter()[omegaIndex()] = 1.e-9;   
}
...
TrkCircleTraj::position( double f) const
{
  ...
  return HepPoint(xt, yt, referencePoint().z());
}
...
TrkCircleTraj::direction( double f) const
{
  ...
  return Hep3Vector( cos(alpha), sin(alpha), 0);
}
...
{
  return pow(6.*tol/sqr(omega()), 0.33333333);
}

void
TrkCircleTraj::getInfo(double flt, HepPoint& pos, Hep3Vector& dir,
...
void
TrkCircleTraj::getInfo( double fltLen, HepPoint& pos, Hep3Vector& dir ) const
{
  double sphi0 = sin(phi0());
  double cphi0 = cos(phi0());
...
void
TrkCircleTraj::getDFInfo2(double fltLen, DifPoint& pos, DifVector& dir) const
{
...
void
TrkCircleTraj::paramFunc(const HepPoint& oldpoint,const HepPoint& newpoint,
