...
//      $Id: TrkDifLineTraj.cc 104 2010-01-15 12:13:14Z stroili $
//
// Description:
//
// Environment:
...
TrkDifLineTraj::position( double f) const
{
  ...
  double cp = cos(phi0());
  double sp = sin(phi0());
  return HepPoint(-d0()*sp + f*cp*cosd + referencePoint().x(),
...
void
TrkDifLineTraj::getInfo(double fltLen, HepPoint& pos, Hep3Vector& dir,
...
void
TrkDifLineTraj::getInfo( double fltLen, HepPoint& pos, Hep3Vector& dir ) const
{
  //  This could be made much more efficient!!!!!
  pos = position(fltLen);
  dir = direction(fltLen);
...
//  Go through the parameters
  switch (idirect) {
  case theta1:
    ddflct[tanDipIndex()][0] = 1.0/sqr(cosd);
...
void
TrkDifLineTraj::paramFunc(const HepPoint& oldpoint,const HepPoint& newpoint,
                          const HepVector& oldvec,const HepSymMatrix& oldcov,
                          HepVector& newvec,HepSymMatrix& newcov,
                          double fltlen)
{
// not yet implemented
  ErrMsg(fatal) << "TrkDifLineTraj::paramFunc() is not implemented!" << endmsg;
}
