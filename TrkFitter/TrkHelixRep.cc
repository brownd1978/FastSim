...
//      $Id: TrkHelixRep.cc 104 2010-01-15 12:13:14Z stroili $
//
// Description:
...
TrkHelixRep::TrkHelixRep(const TrkExchangePar& inPar, TrkRecoTrk* trk,
                         PdtPid::PidType hypo, const TrkHotList &hots) :
  TrkSimpleRep(hots, trk, hypo),
  _traj(inPar)
{}

TrkHelixRep::TrkHelixRep(const TrkExchangePar& inPar, TrkRecoTrk* trk,
                         PdtPid::PidType hypo, TrkHotList &hots, bool stealHots) :
  TrkSimpleRep(hots, trk, hypo,stealHots),
  _traj(inPar)
{}
...
TrkHelixRep::helix(double ) const
{
  ...
  outPar[HelixTraj::phi0Index] = BbrAngle(outPar[HelixTraj::phi0Index]).rad();
...
TrkHelixRep::print(ostream& ostr) const
{
  ostr << "phi0: " << BbrAngle(_traj.phi0()).rad()
       << " om: "<<_traj.omega()
       << " d0: " << _traj.d0() << " z0: " << _traj.z0()
       << " ct: " << _traj.tanDip() << endl;
}
