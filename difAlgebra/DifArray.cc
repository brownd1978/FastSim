...
DifArray::DifArray(const CLHEP::HepVector & a,int npar):
  _nElem(a.num_row()),
...
CLHEP::HepMatrix DifArray::jacobian()const {
  int npar=_pointer[0].nPar();
  CLHEP::HepMatrix temp(nElem(),npar,0);
...
void DifArray::copy(const CLHEP::HepVector& a,int npar){
