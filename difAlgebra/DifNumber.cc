...
double DifNumber::error(const CLHEP::HepSymMatrix& e)const {
...
void DifNumber::fetchDerivatives(CLHEP::HepVector& v)const {
  assert(v.num_row()==nPar());
  for(int i=1; i<=nPar(); i++) {v(i)=derivative(i);}
}

CLHEP::HepVector DifNumber::derivatives()const{
    CLHEP::HepVector temp(nPar());
    fetchDerivatives(temp);
    return temp;
}

double 
DifNumber::correlation(const DifNumber& b,const CLHEP::HepSymMatrix &e) const {
...
double correlation(const DifNumber& a,const DifNumber& b,const CLHEP::HepSymMatrix& e)	// correlation for specified error
