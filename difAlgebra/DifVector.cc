...
DifVector::DifVector(const CLHEP::Hep3Vector& v)
  :x(v.x()),y(v.y()),z(v.z())
{}
...
CLHEP::HepSymMatrix DifVector::errorMatrix(const CLHEP::HepSymMatrix& e)const {
  CLHEP::HepSymMatrix temp(3);
...
CLHEP::HepMatrix DifVector::jacobian()const{
  int npar=x.nPar();
  CLHEP::HepMatrix temp(3,npar);
